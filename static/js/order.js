...
function createDetermine(orderData) {
  if (orderData == null) {
    ...
    const orderInformationElement =
      document.getElementsByClassName("order-information")[0];
    ...
    const orderNumberContent = document.createTextNode(
      `訂單編號：${orderNumber}`
    );
...
function createAPIElement(orderData) {
  ...
  const attractionImageElement =
    document.getElementsByClassName("attraction-image")[0];
  ...
  const contactPhoneElement =
    document.getElementsByClassName("contact-phone")[0];
